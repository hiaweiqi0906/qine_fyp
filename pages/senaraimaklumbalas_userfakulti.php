...
if (isset($all_appprogram_id[0])) {
   ...
   if ($stmt = $con->prepare("SELECT t2.`NAMA`, t2.PROGRAM_ID, t1.TARIKH_MASA_KEMASKINI FROM `appprogram` t1 LEFT JOIN `program` t2 ON t1.`PROGRAM_ID` = t2.`PROGRAM_ID` WHERE t1.`APPPROGRAM_ID` IN ($whole_arr_str)")) {

      $stmt->execute();
      mysqli_stmt_bind_result($stmt, $nama, $pid, $tk);
   ...
   <section class="teachers">
      ...
      if (isset($all_appprogram_id)) {
         for ($ii = 0; $ii < count($all_program); $ii++) {
            if (strtotime($today_date) >= strtotime(date('Y-m-d', strtotime("+6 months", strtotime($all_tarikh[$ii]))))) {
